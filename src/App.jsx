...
function buildTemplate(m,y){
  const rows=[{id:1,type:"שני חסכוני",d:2},{id:2,type:"חג / אירוע",d:4},{id:3,type:"מצחיק / אפליקציה",d:9},{id:4,type:"דרושים",d:16},{id:5,type:"שני חסכוני",d:30},{id:6,type:"פוסט מבצע"},{id:7,type:"פוסט מבצע"},{id:8,type:"פוסט מבצע"},{id:9,type:"פוסט מבצע"}];
  return rows.map(({id,type,d})=>({id,type,date:d?`${d}.${m}.${String(y).slice(2)}`:null,day:d?dayName(d,m,y):null,copy:"",promoText:"",image:null,approved:false,clientNote:""}));
}
...
function SharedView({ganttData}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#F1F5F9",direction:"rtl",fontFamily:"system-ui"}}>
      <header style={{background:HDR,padding:"12px 24px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{color:"rgba(255,255,255,0.8)",fontSize:13}}>עברו על הפוסטים, אשרו או הוסיפו הערות, ולחצו שלח בסוף</div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{textAlign:"left"}}>
            <div style={{color:WH,fontWeight:800,fontSize:15}}>גאנט {MHE[month]} {year} — דלק Ten</div>
          </div>
      ...
      <main style={{maxWidth:900,margin:"24px auto",padding:"0 16px"}}>
        ...
        {!sent?(
          ...
          <div style={{background:"#DCFCE7",border:"1px solid #86EFAC",borderRadius:14,padding:20,textAlign:"center",marginTop:16,color:"#166534",fontWeight:700,fontSize:16}}>
            ✅ הפידבק נשלח! תודה
          </div>
        )}
    ...
    <div style={{background:WH,borderRadius:16,boxShadow:"0 2px 12px rgba(0,0,0,0.06)",overflow:"hidden",marginBottom:16}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        <thead><tr style={{background:"#F8FAFC"}}>
          {["#","תאריך","יום","סוג פוסט","סטטוס"].map(h=><th key={h} style={{padding:"8px 12px",textAlign:"right",fontSize:11,color:"#64748B",fontWeight:700,borderBottom:`1px solid ${BR}`}}>{h}</th>)}
        </tr></thead>
        <tbody>{posts.map(p=>(
          <tr key={p.id} onClick={setOpenRow?()=>setOpenRow(r=>r===p.id?null:p.id):undefined}
            ...
            <td style={{padding:"8px 12px",textAlign:"center",fontSize:15}}>{p.copy?"✅":p.type==="פוסט מבצע"?"⭕":"⏳"}</td>
          </tr>
        ))}</tbody>
      </table>
    </div>
...
function SavedModal({onClose,onLoad}){
  ...
  useEffect(()=>{listGantts().then(d=>{setList(d);setLoading(false);});},[]);
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,direction:"rtl"}}>
      <div style={{background:WH,borderRadius:20,padding:28,width:"90%",maxWidth:480,maxHeight:"70vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
          <h3 style={{color:DK,fontWeight:800}}>📂 גאנטים שמורים</h3>
          <button onClick={onClose} style={{background:"#F1F5F9",border:"none",width:28,height:28,borderRadius:"50%",cursor:"pointer"}}>✕</button>
        </div>
        {loading?<div style={{textAlign:"center",color:"#64748B"}}>טוען...</div>:
          list.length===0?<div style={{textAlign:"center",color:"#64748B"}}>אין גאנטים שמורים</div>:
          list.map(g=>(
            <div key={g.id} onClick={()=>{onLoad(g.data);onClose();}}
              style={{padding:"12px 14px",border:`1px solid ${BR}`,borderRadius:10,marginBottom:8,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{fontWeight:700,color:DK}}>{MHE[g.data?.month]} {g.data?.year}</div>
                <div style={{fontSize:11,color:"#94A3B8"}}>{new Date(g.created_at).toLocaleDateString("he-IL")}</div>
              </div>
              <span style={{color:BL,fontSize:12,fontWeight:700}}>טען ←</span>
            </div>
          ))
        }
      </div>
...
useEffect(()=>{const onPop=()=>setView(window.location.pathname==="/gantt"?"setup":"select");window.addEventListener("popstate",onPop);return()=>window.removeEventListener("popstate",onPop);},[]);
  ...
  useEffect(()=>{
    if(posts.length>0){
      try{sessionStorage.setItem("ganttState",JSON.stringify({view,month,year,extraCtx,posts,shareKey}));}catch{}
    }
  },[view,month,year,extraCtx,posts,shareKey]);
  ...
  async function buildGantt(){
    ...
    setPosts(tmpl); setView("gantt"); setGenerating(true); setGenProgress(0);
    const updated=[...tmpl];
    for(let i=0;i<5;i++){
      const p=updated[i];
...
const Header=({subtitle,showBtns})=>(
  ...
  if(view==="setup")return(
    <div style={{minHeight:"100vh",background:"#F1F5F9",direction:"rtl",fontFamily:"system-ui"}}>
      <Header/>
      {showSaved&&<SavedModal onClose={()=>setShowSaved(false)} onLoad={data=>{setMonth(data.month);setYear(data.year);setExtraCtx(data.extraCtx||"");setPosts(data.posts||[]);setView("gantt");}}/>}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#F1F5F9",direction:"rtl",fontFamily:"system-ui"}}>
      ...
      {showSaved&&<SavedModal onClose={()=>setShowSaved(false)} onLoad={data=>{setMonth(data.month);setYear(data.year);setExtraCtx(data.extraCtx||"");setPosts(data.posts||[]);setView("gantt");setShowSaved(false);}}/>}
