        ...
        <div style={{flex:1,padding:16,minWidth:260}}>
          ...
          {isSharedView&&(
            <div style={{display:"flex",gap:6,marginTop:10}}>
              <button onClick={()=>onUpdate({...post,approved:!post.approved})} style={{background:post.approved?"#16A34A":"#F1F5F9",color:post.approved?WH:DK,border:`1px solid ${post.approved?"#16A34A":BR}`,padding:"6px 14px",borderRadius:7,fontWeight:700,cursor:"pointer",fontSize:12}}>
                {post.approved?"✅ מאושר":"✅ מאושר"}
              </button>
              <input value={post.clientNote||""} placeholder="✏️ יש הערה..." style={{flex:1,padding:"5px 8px",borderRadius:7,border:`1px solid ${BR}`,fontSize:11,fontFamily:"system-ui"}}
