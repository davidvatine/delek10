  ...
  return(
    <div style={{background:WH,borderRadius:16,border:`1px solid ${BR}`,overflow:"hidden",marginBottom:12}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap"}}>
        {/* תמונה */}
        <div style={{width:230,padding:14,background:"#F9FAFB",display:"flex",flexDirection:"column",alignItems:"center",gap:8,borderLeft:`1px solid ${BR}`,flexShrink:0}}>
          <div style={{fontSize:11,color:"#64748B",fontWeight:700}}>🖼️ תמונה לפוסט</div>
          {img?(
            <img src={img} style={{width:200,height:200,objectFit:"contain",borderRadius:8,cursor:"pointer",background:"#F8FAFC"}} onClick={()=>fileRef.current?.click()}/>
          ):(
            <div onClick={()=>fileRef.current?.click()} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f)handleImg(f);}}
              style={{width:200,height:160,border:"2px dashed #CBD5E1",borderRadius:10,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#94A3B8",fontSize:11,textAlign:"center",cursor:"pointer"}}>
