...
export function buildPrompt(type,ctx,date,day,promoText=""){
  ...
  const RULES=`
כללי ברזל לכתיבת הפוסט:
1. כתוב אך ורק בעברית תקינה ומדויקת - ללא שגיאות כתיב, פיסוק או הגהה.
2. אסור להשתמש במקף הארוך (—). אם צריך מקף, רק מקף רגיל (-).
3. לא יותר מ-3 אימוג'ים בכל הפוסט.
4. הפוסט חייב להיות קשור לרכב, דלק, חשמל או תחנת הדלק - לא לחיים בכלל.
5. אסור לפתוח ב"אביב הגיע", "הקיץ כאן", "החורף בא" או כל משפט עונתי שחוק.
6. הפתיחה חייבת לבנות עניין לפני שמגיעים לפרסום - שאלה, מצב מוכר, הומור עדין.
7. סגנון: חברי, אישי, כמו חבר שמספר לחבר. לא שיווקי-רשמי.
8. אורך: קצר וחד. לא יותר מ-15 שורות.
9. אל תחזור על אותו מבנה מפעם לפעם - הפוסט חייב להרגיש טרי.
...
כתוב כעת פוסט לחג: ${HL}. קשור לתחנת הדלק, לרכב או לנסיעה.
סיים ב: ${DISCLAIMER}`;
...
- קשר לנהיגה, רכב, תחנת דלק
- קריאה לתגובות (לא חובה)
- סיים: ${APP_LINK}`;
