...
const RESEND_KEY = process.env.RESEND_API_KEY || "re_bxJzgT6R_EwjfTxkpnJvhWpvydaAPEE9f6";
...
export default async function handler(req, res) {
  ...
  <div dir="rtl" style="font-family:Arial;max-width:500px;margin:0 auto">
    <div style="background:#1565C0;padding:16px 20px;border-radius:8px 8px 0 0">
      <h2 style="color:white;margin:0;font-size:18px">💫 חצרה חלהץ מההוה</h2>
    </div>
    <div style="background:#f5f5f5;padding:20px;border-radius:0 0 8px 8px">
      <p><strong>דאץר:</strong> ${gantt_id}</p>
      <p><strong>גצע:</strong> ${post_type || 'כ^Wיל'}</p>
      ${author_name ? `<p><strong>מאר:</strong> ${author_name}</p>` : ''}
